...
class Trainer(embedder):
    ...
    def evaluate1(self, model, k, is_valid='test'):
        ...
        for _, batch in enumerate(loader):
            ...
            #存储下来
            # torch.save(tensor_data, 'tensor_musicdata.pt')

            # is_in_active = torch.tensor([element in active_items for element in top_item], dtype=torch.int, device = 'cuda')
            # is_in_inactive = torch.tensor([element in inactive_items for element in top_item], dtype=torch.int, device = 'cuda')
        ...
        with open('melt-sasrec-music.txt', 'a') as file:
