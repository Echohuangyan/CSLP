...
class Trainer(embedder):
    ...
    def train(self):
        ...
        self.model = CSLP(self.args, self.logger, self.train_data, self.device, self.item_num, u_L_max, i_L_max).to(self.device)
        ...
        for epoch in range(self.args.e_max):
            ...
            for _, ((u,seq,pos,neg),(u_idx), (i_idx)) in enumerate(zip(train_loader, u_h_loader, i_h_loader)):
                ...
                loss = self.model(u, seq, pos, neg, u_idx, i_idx, self.user_context, self.item_context, self.n_item_context, \
                                  self.user_threshold, self.item_threshold, epoch, tailidx)
                loss.backward()
        ...
        self.validcheck.print_result()
        self.logger.info(f"{self.args.lamb_u},{self.args.lamb_i}")
        # self.evaluate1(self.validcheck.best_model, k=10, is_valid='test')
    ...
    def test(self):
        ...
        self.model = CSLP(self.args, self.logger, self.train_data, self.device, self.item_num, user_max_thres, item_max_thres, test=True).to(self.device)
        self.inference_negative_sampler = NegativeSampler(self.args, self.dataset)
    ...
    def evaluate1(self, model, k, is_valid='test'):
        """
        ...
        df = pd.read_csv('dataset/%s.txt' %self.args.dataset, header='infer', sep=' ', names=['user', 'item', 'time'])
        fre_item = df['item'].value_counts().index.values.tolist()

        # value_counts = df['item'].value_counts().reset_index()
        # value_counts.columns = ['itemid', 'Count']
        # sorted_counts = value_counts.sort_values(by='Count', ascending=False)
        # sorted_ids = sorted_counts['itemid'].tolist()

        # print(sorted_ids)
        # print(a)
        active_idx = int(len(fre_item) * 0.2)
        ...
        for _, (u, seq, item_idx, test_idx) in enumerate(loader):
            ...
            predictions = -model.sasrec.predict(seq.numpy(), item_idx.numpy(), u_tail,
                                                model.user_branch.W_U)  # Sequence Encoder
            # rank = predictions.argsort(1).argsort(1)[:, 0].cpu().numpy()
            # top_indices = predictions.argsort(1)[:, :10]#每行前十名的列索引
            # top_indices = top_indices.numpy()
            predictions = predictions.to('cuda')
            top_indices = predictions.argsort(1)[:, :10].cpu().numpy().flatten()
            # row_indeces = np.arange(len(predictions))#行索引
            row_indeces = np.array([i for i in range(len(predictions)) for _ in range(10)])
            result_indices = (row_indeces, top_indices)#提取出前10的行列索引

            #根据行列索引提取itemid
            top_item = item_idx[result_indices]

            is_in_active = torch.tensor([element in active_items for element in top_item], dtype=torch.int)
            is_in_inactive = torch.tensor([element in inactive_items for element in top_item], dtype=torch.int)

            count_A += torch.sum(is_in_active)
            count_B += torch.sum(is_in_inactive)
            # count += torch.sum(predictions)

        count = count_A + count_B
        pcount_A = count_A/count
        pcount_B = count_B/count
        print(count)
        print(count_A)
        print(count_B)
        print({'rsp': {'Head_Item': pcount_A, 'Tail_Item': pcount_B}})
...
#PMusic.txt 改数据集需要改range
# df = pd.read_csv('dataset/PMusic.txt', header='infer', sep=' ', names=['user', 'item', 'time', 'p'])
# dict = {}
# for i in range(132595):#394908  132595
#     key = df.loc[i, 'item']
#     value = df.loc[i, 'p']
#     dict[key] = value
...
class CSLP(torch.nn.Module):
    ...
    def forward(self, u, seq, pos, neg, u_h_idx, i_h_idx, user_context, item_context, n_item_context, user_thres, item_thres, epoch, tailidx):
        ...
        pos_logits, neg_logits = self.sasrec(u, seq, pos, neg) #torch.Size([128, 50])
        pos_labels, neg_labels = torch.ones(pos_logits.shape).to(self.device), torch.zeros(neg_logits.shape).to(self.device)
        indices = np.where(pos != 0)#查找数组pos中非0元素的索引，返回为行索引和列索引array(array(),array())

        p = self.ptail_1(pos, tailidx)
        p = torch.tensor(p)
        p = p.to(self.device)
        # print(p)

        nindices = np.where(neg != 0)
        n = self.ptail_1(neg, tailidx)
        n = torch.tensor(n)
        n = n.to(self.device)

        # for i in range(len(pos_logits[indices])):
        #     lossi = torch.mul((self.bce_criterion(pos_logits[indices][i], pos_labels[indices][i])),p)
        #     print(lossi)
        #     batch_loss += lossi
        losspre = torch.mul((self.bce_criterion(pos_logits[indices], pos_labels[indices])), torch.reciprocal(p)).mean()
        # losspre += self.bce_criterion(neg_logits[indices], neg_labels[indices])
        losspre += torch.mul((self.bce_criterion(neg_logits[nindices], neg_labels[nindices])), torch.reciprocal(n)).mean()
        # print(losspre)
        loss = user_loss * self.args.lamb_u + item_loss * self.args.lamb_i + losspre

        # loss = item_loss * self.args.lamb_i + losspre
        # loss = user_loss * self.args.lamb_u + losspre
