...
class Trainer(embedder):
    ...
    def evaluate1(self, model, k, is_valid='test'):
        ...
        for _, (u, seq, item_idx, test_idx) in enumerate(loader):
            ...
            top_item = item_idx[result_indices].to('cuda')
...
class SASRec(torch.nn.Module):
    ...
    def log2feats(self, log_seqs):
        ...
        for i in range(len(self.attention_layers)):
            ...
            Q = self.attention_layernorms[i](seqs)#对seqs进行归一化操作
    ...
    def forward(self, user_ids, log_seqs, pos_seqs, neg_seqs):
        ...
        pos_logits = (log_feats * pos_embs).sum(dim=-1)#log_feats.size() torch.Size([128, 50, 50])
        neg_logits = (log_feats * neg_embs).sum(dim=-1)#pos_embs.size() torch.Size([128, 50, 50])
