...
class Trainer(embedder):
    ...
    def ptail_1(self, pos):
        df = pd.read_csv('dataset/P%s.txt' %self.args.dataset, header='infer', sep=' ', names=['user', 'item', 'time', 'p'])
        index = []
        # print(pos)
        for i in range(len(pos)):
            # print(len(pos))
            # print(pos[i])
            # newpos = list(filter(lambda x: x != 0, pos[i]))
            newpos = pos[i].tolist()
            index.append(newpos)
        p = []
        print(index)
        for i in index:
            filtered_values = df.loc[df['item'] == i, 'p'].drop_duplicates().values[0]
            p.append(filtered_values)
        return p
    ...
    def cross_entropy(self, seq_out, pos_ids, neg_ids):
        ...
        p = self.ptail_1(pos_ids)
        p = torch.tensor(p).to(self.device)

        n = self.ptail_1(neg_ids)
        n = torch.tensor(n).to(self.device)
        ...
        pos_loss = torch.mul((- torch.log(torch.sigmoid(pos_logits) + 1e-24)), torch.reciprocal(p)).mean()
        neg_loss = torch.mul((- torch.log(1 - torch.sigmoid(neg_logits) + 1e-24)), torch.reciprocal(n)).mean()
    ...
    def evaluate1(self, model, k, is_valid='test'):
        ...
        tensor_data = torch.Tensor().to('cuda')
        for _, batch in enumerate(loader):
            ...
            top_indices = test_logits.argsort(1)[:, :10].cpu().numpy().flatten()
            ...
            tensor_data = torch.cat((tensor_data, top_item),dim = 0)

        active_items = torch.tensor(fre_item[: active_idx]).to('cuda')
        is_in_active = torch.isin(tensor_data, active_items)
        count_A = is_in_active.sum().item()
        count_B = tensor_data.size(0) - count_A
        ...
        with open('mpbr-fmlp.txt', 'a') as file:
            file.write(f'rsp: Head_Item: {pcount_A}, Tail_Item: {pcount_B},Dataset: {self.args.dataset}')
